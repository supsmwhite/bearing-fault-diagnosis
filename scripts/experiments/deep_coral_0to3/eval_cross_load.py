"""Evaluate Deep CORAL 0 hp -> 3 hp checkpoints."""
...
CNN1D_TARGET_ACCURACY = 0.836735
CNN1D_TARGET_MACRO_F1 = 0.729576
CLASS_NAMES = [
    "Normal",
    "IR007",
    "B007",
...
def evaluate_checkpoint(tag: str, checkpoint_path: Path, config: dict, loader, test_indices, processed_data, device) -> dict:
    ...
    confusion_path = experiment_dir / f"figures/cross_load_confusion_matrix_{tag}.png"
    save_predictions(predictions_path, test_indices, metrics, processed_data)
    save_confusion_matrix(confusion_path, cm, f"Deep CORAL 0->3 {tag} Confusion Matrix")
    save_metrics(metrics_path, checkpoint_path, checkpoint, metrics, cm, report)
    print(f"Deep CORAL 0->3 {tag}:")
...
def main() -> None:
    ...
    loader = DataLoader(
        CWRUWindowDataset(str(processed_path), indices=test_indices, return_meta=False),
