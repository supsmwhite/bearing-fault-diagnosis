"""Train Deep CORAL for 0 hp -> 3 hp adaptation."""
...
def main() -> None:
    ...
    source_train_loader = DataLoader(
        CWRUWindowDataset(str(processed_path), indices=source_train_indices, return_meta=False),
        batch_size=training_config["batch_size"],
        shuffle=True,
        drop_last=True,
    )
    source_val_loader = DataLoader(
        CWRUWindowDataset(str(processed_path), indices=source_val_indices, return_meta=False),
        batch_size=training_config["batch_size"],
        shuffle=False,
    )
    target_train_loader = DataLoader(
        CWRUWindowDataset(str(processed_path), indices=target_train_indices, return_meta=False),
        batch_size=training_config["batch_size"],
        shuffle=True,
        drop_last=True,
    )
    ...
    for epoch in range(1, training_config["epochs"] + 1):
        total_loss, class_loss, coral, train_acc, train_f1 = train_one_epoch_deep_coral(
            ...
            optimizer,
            device,
            coral_loss_weight=training_config["coral_loss_weight"],
        )
        ...
        print(
            ...
            f"coral_loss={coral:.6f} source_train_accuracy={train_acc:.6f} "
            f"source_train_macro_f1={train_f1:.6f} | "
