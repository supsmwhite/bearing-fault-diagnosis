"""Evaluate trained Deep CORAL checkpoints on the target 2 hp test split."""
...
DANN_BEST_TARGET_ACCURACY = 0.889968
DANN_BEST_TARGET_MACRO_F1 = 0.831587
DANN_FINAL_TARGET_ACCURACY = 0.864078
DANN_FINAL_TARGET_MACRO_F1 = 0.820324
CLASS_NAMES = [
    "Normal",
    "IR007",
    "B007",
...
def safe_load_checkpoint(checkpoint_path, device):
    """Load a trusted local checkpoint with PyTorch version compatibility."""
    try:
        return torch.load(
            checkpoint_path,
            map_location=device,
...
@torch.no_grad()
def evaluate_deep_coral(model, dataloader, device):
    model.eval()
    y_true_batches = []
    y_pred_batches = []
...
def evaluate_checkpoint(
    ...
    model = DeepCORAL1D(num_classes=config["experiment"]["num_classes"]).to(device)
    checkpoint = safe_load_checkpoint(checkpoint_path, device)
    model.load_state_dict(checkpoint["model_state_dict"])

    metrics = evaluate_deep_coral(model, test_loader, device)
    ...
    save_confusion_matrix(confusion_matrix_path, cm, f"Deep CORAL {tag} Cross-load Confusion Matrix")
...
def main() -> None:
    ...
    print("Comparison:")
    print(f"CNN1D source-only target_accuracy = {CNN1D_TARGET_ACCURACY:.6f}")
    print(f"CNN1D source-only target_macro_f1 = {CNN1D_TARGET_MACRO_F1:.6f}")
    print(f"DANN best target_accuracy = {DANN_BEST_TARGET_ACCURACY:.6f}")
    print(f"DANN best target_macro_f1 = {DANN_BEST_TARGET_MACRO_F1:.6f}")
    print(f"DANN final target_accuracy = {DANN_FINAL_TARGET_ACCURACY:.6f}")
    print(f"DANN final target_macro_f1 = {DANN_FINAL_TARGET_MACRO_F1:.6f}")
    print(f"Deep CORAL best target_accuracy = {best_metrics['accuracy']:.6f}")
    print(f"Deep CORAL best target_macro_f1 = {best_metrics['macro_f1']:.6f}")
    print(f"Deep CORAL final target_accuracy = {final_metrics['accuracy']:.6f}")
    print(f"Deep CORAL final target_macro_f1 = {final_metrics['macro_f1']:.6f}")
