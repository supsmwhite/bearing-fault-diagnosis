"""Analyze CNN1D source-only gap-split cross-load prediction errors."""
...
def analyze(predictions_path: Path, analysis_path: Path, per_class_path: Path) -> None:
    ...
    text = [
        "CNN1D gap-split cross-load error analysis",
        "",
...
def main() -> None:
    ...
    analyze(
        experiment_dir / "logs/cross_load_predictions.csv",
        Path(config["outputs"]["error_analysis"]),
        experiment_dir / "logs/cross_load_per_class_metrics.csv",
    )
