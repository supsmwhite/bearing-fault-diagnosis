"""Build CWRU windows without per-window z-score normalization.

This ablation applies one z-score normalization per source file before
windowing, so each window keeps its relative amplitude within that file.
"""
...
OUTPUT_PATH = PROJECT_ROOT / "data/processed/cwru_12k_de_windows_no_window_zscore.npz"
WINDOW_SIZE = 1024
STRIDE = 512
EPS = 1e-8
...
def file_level_zscore(signal: np.ndarray) -> np.ndarray:
...
def main() -> None:
    ...
    for item in config["files"]:
        ...
        signal = file_level_zscore(signal)
    ...
    np.savez(
        ...
        normalization="file_level_zscore_no_window_zscore",
    )

    print("Normalization: file-level z-score, no per-window z-score")
    print(f"Total windows: {X.shape[0]}")
    print(f"X shape: {X.shape}")
    print(f"y shape: {y.shape}")
    print(f"Load distribution: {dict(Counter(load_hp.tolist()))}")
    print(f"Class distribution: {dict(Counter(class_name.tolist()))}")
